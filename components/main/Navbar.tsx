...
const Navbar = () => {
  ...
  const debouncedScrollToSection = useCallback((sectionId: string) => {
    ...
    if (targetSection) {
      ...
      // Get the navbar height with a small buffer
      const navbarHeight = (document.querySelector('nav')?.offsetHeight || 70) + 20;
      
      // Update active section immediately for better UX
      setActiveSection(sectionId);
      
      // Get section position
      const targetPosition = window.scrollY + targetSection.getBoundingClientRect().top;
      
      // Add a small delay to ensure any animations/transitions have completed
      setTimeout(() => {
        // Smooth scroll animation with adjusted offset
        gsap.to(window, {
          duration: 1,
          scrollTo: {
            y: targetPosition - navbarHeight,
            autoKill: false
          },
          ease: "power2.inOut",
          onComplete: () => {
  ...
  }, [setIsScrolling, setActiveSection]);
